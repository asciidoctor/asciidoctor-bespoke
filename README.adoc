...
A converter for {uri-asciidoctor}[Asciidoctor] that generates the HTML component of a {uri-bespoke}[Bespoke.js] presentation from AsciiDoc.
...
Naturally, you'll also need to setup a Bespoke.js project, just like you would for any other Bespoke.js presentation.
...
We plan to provide a Yeoman generator that integrates {project-name} automatically and is kept current.
...
gulp.task('html', ['clean:html'], function() {
  return gulp.src('src/index.adoc')
    ...
    .pipe(exec('asciidoctor-bespoke -o - src/index.adoc', { pipeStdout: true })) // <1>
